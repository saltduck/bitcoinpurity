...
BitcoinUnit BitcoinUnits::FromSetting(const QString& s, BitcoinUnit def)
{
    ...
    // Former tonal (TBC) settings fall back to the default decimal unit.
    if (s == "4" || s == "5" || s == "bTBC" || s == "sTBC" || s == "TBC") return def;
    return def;
}

QDataStream& operator<<(QDataStream& out, const BitcoinUnit& unit)
{
    return out << std::get<qint8>(BitcoinUnits::ToSetting(unit));
}
